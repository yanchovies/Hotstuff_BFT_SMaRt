...
public class MessageHandler {
    ...
    protected void processData(SystemMessage sm) {
        if (sm instanceof ConsensusMessage) {
            ...
            if (tomLayer.controller.getStaticConf().getUseMACs() == 0 || consMsg.authenticated || consMsg.getSender() == myId) backup.deliver(consMsg);
                ...
                try {
                    ...
                    System.out.println("Failed to serialize consensus message"+ex);
                }
                ...
                try {
                    ...
                    System.out.println("Failed to generate MAC"+ex);
                }
        ...
        }else if(sm instanceof KeyShareMessage){
            KeyShareMessage keyShare = (KeyShareMessage)sm;
            if(keyShare.isLeaderMessage()){
                backup.setKeyShare(keyShare);
            }else {
                primary.verifyKey(keyShare);
            }
        }else if(sm instanceof SigShareMessage){
            SigShareMessage sigShare = (SigShareMessage) sm;
            primary.deliver(sigShare);
        }else if(sm instanceof NewViewMessage){
            NewViewMessage newViewMessage = (NewViewMessage) sm;
            primary.newViewReceived(newViewMessage);
        }else{
