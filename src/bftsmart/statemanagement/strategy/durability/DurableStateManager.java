...
public class DurableStateManager extends BaseStateManager {
	...
	protected void requestState() {
		...
		System.out.println("I just sent a request to the other replicas for the state up to CID "
				+ waitingCID);
	...
	public void SMRequestDeliver(SMMessage msg, boolean isBFT) {
		...
		if (SVController.getStaticConf().isStateTransferEnabled()
			...
			System.out.println("State asked by replica " + msg.getSender());
	...
	public void SMReplyDeliver(SMMessage msg, boolean isBFT) {
		...
		if (SVController.getStaticConf().isStateTransferEnabled()) {
			logger.debug("The state transfer protocol is enabled");
			System.out.println("I received a state reply for CID "
					+ reply.getCID()
					+ " from replica "
					+ reply.getSender());
			...
			if (waitingCID != -1 && reply.getCID() == waitingCID) {
				...
				try {
					...
					System.out.println("Failed to connect to address"+e);
				} catch (IOException e) {
					// TODO Auto-generated catch block
					System.out.println("Failed to connect to address"+e);
				} catch (ClassNotFoundException e) {
					// TODO Auto-generated catch block
					System.out.println("Failed to deserialize application state object"+e);
				}
				...
				if (senderStates.size() == 3) {
					...
					if (Arrays.equals(stateCkp.getHashLogLower(), lowerLogHash))
						...
						System.out.println("Lower log does not match");
					// validate upper log
					if (!haveState || !Arrays.equals(stateCkp.getHashLogUpper(), upperLogHash)) {
						haveState = false;
						System.out.println("Upper log does not match");
					}
