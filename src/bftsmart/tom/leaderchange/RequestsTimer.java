...
public class RequestsTimer {
    ...
    public void run_lc_protocol() {
        ...
        if (!pendingRequests.isEmpty()) {
            ...
            System.out.println("The following requests timed out: " + pendingRequests);
            
            for (ListIterator<TOMMessage> li = pendingRequests.listIterator(); li.hasNext(); ) {
                TOMMessage request = li.next();
                if (!request.timeout) {

                    System.out.println("Forwarding requests "+request+" to leader");
            ...
            if (!pendingRequests.isEmpty()) {
                System.out.println("Attempting to start leader change for requests "+pendingRequests);
                //Logger.debug = true;
    ...
    class SendStopTask extends TimerTask {
        ...
        public void run() {
                ...
                System.out.println("Re-transmitting STOP message to install regency " + stop.getReg());
