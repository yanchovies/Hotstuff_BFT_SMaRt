...
public abstract class DurabilityCoordinator implements Recoverable, BatchExecutable {
	...
	public DurabilityCoordinator() {
		try {
			...
			System.out.println("Failed to get message digest object"+ex);
		}
	}
	...
	public int setState(ApplicationState recvState) {
		...
		if (recvState instanceof CSTState) {
			...
			System.out.println("Installing log from " + (lastCheckpointCID+1) + " to " + lastCID);
			...
			for (int cid = lastCheckpointCID + 1; cid <= lastCID; cid++) {
				try {
					...
					System.out.println("Failed to process and verify batched requests"+e);
				}
