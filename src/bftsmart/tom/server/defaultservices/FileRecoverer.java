...
//	}
	...
	public CommandsInfo[] getLogState(int index, String logPath) {
		...
		if ((log = openLogFile(logPath)) != null) {
			...
			try {
				...
				System.out.println("Failed to get state log"+e);
			}
	...
	public byte[] getCkpState(String ckpPath) {
		...
		if ((ckp = openLogFile(ckpPath)) != null) {
			...
			try {
				...
				System.out.println("Failed to get checkpoint"+e);
			}
	...
	public void recoverCkpHash(String ckpPath) {
		...
		if ((ckp = openLogFile(ckpPath)) != null) {
			...
			try {
				...
				System.out.println("State recover was aborted due to an unexpected exception"+e);
			}
		...
		try {
			...
			while (mayRead) {
				try {
					...
					System.out.println("Failed to recover from checkpoint"+e);
					ckp = null;
					mayRead = false;
				}
			}
	...
	private RandomAccessFile openLogFile(String file) {
		try {
			...
			System.out.println("Failed to open log file"+e);
		}
		return null;
	}
		...
		try {
			...
			while (mayRead) {
				try {
					...
					System.out.println("Failed to recover log state"+e);
					state.clear();
					mayRead = false;
				}
			}
		...
		try {
			...
			while (mayRead) {
				...
				try {
					...
					System.out.println("Failed to recolver log state"+e);
					state.clear();
					mayRead = false;
				}
			}
