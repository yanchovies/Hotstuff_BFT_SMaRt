...
public class Synchronizer {
	...
	// still not in the leader change phase?
        if (lcManager.getNextReg() == lcManager.getLastReg()) {
            ...
            try { // serialize content to send in STOP message
                ...
                // send STOP-message                
                System.out.println("Sending STOP message to install regency " + regency + " with " + (messages != null ? messages.size() : 0) + " request(s) to relay");
                ...
                System.out.println("Could not serialize STOP message"+ex);
            } finally {
                try {
                    out.close();
                    bos.close();
    ...
    private void processOutOfContextSTOPs(int regency) {
        ...
        if (stops.size() > 0) {
            System.out.println("Processing " + stops.size() + " out of context STOPs for regency " + regency);
        } else {
    ...
    private void processSTOPDATA(LCMessage msg, int regency) {
        ...
        try { // deserialize the content of the message
            ...
            System.out.println("Could not deserialize STOPDATA message"+ex);
        }
    ...
    private void processSYNC(byte[] payload, int regency) {
        ...
        try { // deserialization of the message content
            ...
            System.out.println("Could not deserialize SYNC message"+ex);
        }
    }
    ...
    private TOMMessage[] deserializeTOMMessages(byte[] playload) {
        ...
        try { // deserialize the content of the STOP message
            ...
            System.out.println("Could not serialize requests"+ex);
        }
    ...
    private void startSynchronization(int nextReg) {
        ...
        if (condition && lcManager.getNextReg() == lcManager.getLastReg()) {
            ...
            try { // serialize conent to send in the STOP message
                ...
                System.out.println("Could not deserialize STOP message"+ex);
            }
        }
        ...
        if (condition && lcManager.getNextReg() > lcManager.getLastReg()) {
            ...
            if (leader != this.controller.getStaticConf().getProcessId()) {
                ...
                try { // serialize content of the STOPDATA message
                    ...
                    System.out.println("Sending STOPDATA of regency " + regency);
		...
		//TODO: Turn on timeout again?
                ...
                if (sync.size() > 0) {
                    System.out.println("Processing out of context SYNC for regency " + regency);
                } else {
                    System.out.println("No out of context SYNC for regency " + regency);
                }
    ...
    public void deliverTimeoutRequest(LCMessage msg) {
        ...
        switch (msg.getType()) {
            case TOMUtil.STOP: { // message STOP
                ...
                System.out.println("Last regency: " + lcManager.getLastReg() + ", next regency: " + lcManager.getNextReg());
    ...
    private void catch_up(int regency) {
		...
		//TODO: Missing: serialization of the proof?
                ...
                System.out.println("Sending SYNC message for regency " + regency);
    ...
    private void finalise(int regency, CertifiedDecision lastHighestCID,
        ...
        if (tom.getLastExec() + 1 < lastHighestCID.getCID()) { // is this a delayed replica?
            ...
            System.out.println("NEEDING TO USE STATE TRANSFER!! (" + lastHighestCID.getCID() + ")");
...
/*****************************************************7.13*************************************************************/
//            execManager.getStoppedMsgs().add(backup.getFactory().createPrepare(currentCID, 0, propose));
/******************************************************************************************************************/
...
//            }
        ...
        if (e != null){//e != null) {
            ...
            System.out.println("Installed proof of last decided consensus " + lastHighestCID.getCID());
            ...
            if (tom.getLastExec() + 1 == lastHighestCID.getCID()) {
                ...
                System.out.println("I'm still at the CID before the most recent one!!! (" + lastHighestCID.getCID() + ")");
                cons.decided(e, true);
            }
            else {
            ...
            System.out.println("I did not install any proof of last decided consensus " + lastHighestCID.getCID());
        }
        cons = null;
        e = null;
...
/*******************************************7.13************************************************************/
            ...
            if (iAmLeader) {
                ...
                System.out.println("Sending PREPARE message for CID " + currentCID + ", timestamp " + e.getTimestamp() + ", value " + Arrays.toString(e.propValueHash));
                ...
                BigInteger n = keyShareMessages[controller.getStaticConf().getProcessId()].getN();
                BigInteger ee = keyShareMessages[controller.getStaticConf().getProcessId()].getE();
                execManager.getPrimary().setEandN(e.propValue,ee,n);
                for(int i=0;i<keyShareMessages.length;i++){
                    keyShareMessages[i].setSender(me);
                    communication.send(new int[]{i},keyShareMessages[i]);
                }
                execManager.getPrimary().startConsensus(currentCID,e.propValue);
                /********************************************************************************************************/
