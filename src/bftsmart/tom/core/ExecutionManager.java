...
public final class ExecutionManager {
    ...
    private Backup backup; // backup role
    private Primary primary; // primary role
     ...
     */
    public ExecutionManager(ServerViewController controller, Primary primary,
                            Backup backup, int me) {
        //******* EDUARDO BEGIN **************//
        ...
        // Get initial leader
        if (controller.getCurrentViewAcceptors().length > 3){
    ...
    public void restart() {
        ...
        while (!stoppedMsgs.isEmpty()) {
            ...
            if (pm.getNumber() > tomLayer.getLastExec()) backup.processMessage(pm);
        }
        stoppedMsgsLock.unlock();
        ...
        if (isRetrievingState || // Is this replica retrieving a state?
            ...
            if (stopped) {//just an optimization to avoid calling the lock in normal case
                ...
                if (isRetrievingState || 
                   ...
                   /*************************************************7.13**********************************************/
                        ) {//(inExec == -1 && msg.getType() != MessageFactory.PREPARE))) { //not propose message for the next consensus
                    /**********************************************************************************************/
...
//                        else if (msg.getType() == MessageFactory.ACCEPT) countAccepts++;
    ...
    public void addOutOfContextMessage(ConsensusMessage m) {
        ...
        /******* BEGIN OUTOFCONTEXT CRITICAL SECTION *******/
        /*****************************************7.13************************************************/
        if (m.getType() == MessageFactory.PREPARE) {//PROPOSE) {
        /*****************************************************************************************/
