package bftsmart.tom.util.thresholdsig;
...
class SafePrimeGen {
  ...
  public static BigInteger generateStrongPrime(final int bitLength,
    ...
    // ////
    a = t.multiply(BigInteger.valueOf(2L)); // a = t * 2
    // ////
    ...
    do {
      ...
      r = b.add(BigInteger.ONE); // r = b + --> r = 2 * i * t + 1
      i = i.add(BigInteger.ONE); // increment i --> i = i + 1
      if (log.isLoggable(Level.FINEST))
    ...
    outerloop: do {
      ...
      // ////
      a = s.multiply(BigInteger.valueOf(2L)); // a = s * 2
      b = r.subtract(BigInteger.valueOf(2L)); // b = r - 2
      c = s.modPow(b, r); // c = s^b mod r --> s^(r-2) mod r
      d = c.multiply(a); // d = c * a --> 2 * (s^(r-2) mod r) * s
      p0 = d.subtract(BigInteger.ONE); // p0 = d - 1 --> 2 * (s^(r-2) mod r) *
      // s - 1
      if (log.isLoggable(Level.FINEST))
        log
      ...
      // we use p0 + a * j
      // ////
      a = BigInteger.valueOf(2L).multiply(r).multiply(s); // a = 2 * r * s
      ...
      // 2^(bitLength-1)
      // - p0
      c = BigInteger.valueOf(2L).multiply(r).multiply(s); // c =
      // 2*r*s
      ...
      // ////
      if (b.mod(c).equals(BigInteger.ZERO) == false) {
